...
class _ChangeNameState extends State<ChangeName> {
  ...
  Future<void> getUserData() async {
    ...
    await ref.child('nameChange').once().then((DataSnapshot snapshot) {
      setState(() {
        retrieved = snapshot.value;         // date that you change the name 

        retrievedfiftyDaysFromNow = DateTime.parse("$retrieved");       //convert to Date Time obeject    

        today = DateTime.now();                //date and time now
        print(today);                          
        print(retrievedfiftyDaysFromNow);
        different = retrievedfiftyDaysFromNow.difference(today).inDays;      //difference between today and last date that you change the name
        print(different);
      });
    });
    _loading = false;
                ...
                child: ListView(
                  children: [
                    ...
                    TextFormField(                                            //First name
                      readOnly: different <= (0) ? false : true,              //you can write on the text field if diffent<=0
                    ...
                    RaisedButton(
                      ...
                      onPressed: () {
                        ...
                        if (different <= 0) {                    
                          today = DateTime.now();               //get the date time now
                          fiftyDaysFromNow =
                              today.add(const Duration(days: 50)).toString();              //add 50 days today date time

                          ref.child("nameChange").set(fiftyDaysFromNow);                     //send to database
                        }
                        Navigator.of(context).push(MaterialPageRoute(
                            builder: (BuildContext context) => SettingsPage()));
                      },
